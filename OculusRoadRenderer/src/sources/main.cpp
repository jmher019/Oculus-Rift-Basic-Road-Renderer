...
static bool MainLoop(bool retryCreate) {
	...
	for (int eye = 0; eye < 2; ++eye)
	{
		...
		eyeDepthBuffer[eye] = new DepthBuffer(eyeRenderTexture[eye]->texSize, 0);
	...
	while (Platform.HandleMessages())
	{
		...
		if (sessionStatus.IsVisible)
		{
			...
			for (int eye = 0; eye < 2; ++eye)
			{
				...
				ld.Viewport[eye] = OVR::Recti(eyeRenderTexture[eye]->texSize);
...
int WINAPI WinMain(HINSTANCE hinst, HINSTANCE, LPSTR, int) {
	...
	VALIDATE(Platform.InitWindow(hinst, L"Oculus Road Renderer (GL)"), "Failed to open window.");
